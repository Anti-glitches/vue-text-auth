    ...
    <div v-if="!pageLoading">
        <div v-if="isLoggedin">
            <div v-if="fbUserTxtDoc && selectedDoc === null" class="m-6">
                <div
                    @click="handleNew"
                    class="flex items-center gap-3 p-3 px-8 w-40 border text-white cursor-pointer rounded-full bg-gradient-to-r from-yellow-400 to-orange-500"
                >
                    <i class="fa-solid fa-file-circle-plus text-2xl"></i>
                    <span class="text-lg font-semibold">New</span>
                </div>
                <div
                    class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 cursor-pointer mt-6"
                >
                    <div
                        v-for="(doc, i) in fbUserTxtDoc"
                        :key="i"
                        @click="handleClick(doc, i)"
                        class="w-full border flex gap-4 items-center bg-slate-100 p-3 px-4 rounded-lg shadow"
                    >
                        <i
                            class="fa-solid fa-file-lines text-2xl text-blue-400"
                        ></i>
            ...
            <div v-if="selectedDoc" class="bg-slate-100">
                <div
                    class="bg-white border-b-2 px-24 py-8 mb-4 flex gap-3 justify-between"
                >
                    <i
                        class="fa-solid fa-file-lines text-blue-400 text-5xl"
                    ></i>
                    <div class="flex-grow">
                        <input
                            type="text"
                            v-model="selectedInputTitle"
                            class="font-semibold text-lg"
                        />
                        <p v-if="loading">
                            <i class="fa-solid fa-rotate"></i> Saving changes
                        </p>
                        <p v-else>
                    ...
                    <div
                        @click="handleHome"
                        class="border flex justify-center items-center p-4 rounded-lg shadow cursor-pointer min-w-fit"
                    >
                        <i class="fa-solid fa-angle-left mr-2"></i>
                        <span class="font-semibold">Dashboard</span>
                    </div>
                </div>
                <div class="px-24 h-[60rem]">
                    <textarea
                        placeholder="Type something..."
                        v-model="selectedTextarea"
                        class="p-24 w-full h-full shadow resize-none outline-none"
                    ></textarea>
                </div>
            </div>
        </div>
...
export default {
    setup() {
        ...
        watch(selectedTextarea, (currentValue, oldValue) => {
            if (
                ...
                currentValue != selectedDoc.value.textarea
            ) {
                loading.value = true;
                console.log("changes");
                ...
                setTimeout(async () => {
                    if (temp === selectedTextarea.value) {
                        fbUserTxtDoc.value[id.value].textarea =
        ...
        watch(selectedInputTitle, (currentValue, oldValue) => {
            if (
                currentValue != oldValue &&
                currentValue != selectedDoc.value.inputTitle
            ) {
                loading.value = true;
                ...
                setTimeout(async () => {
                    if (temp === selectedInputTitle.value) {
                        fbUserTxtDoc.value[id.value].inputTitle =
    ...
    components: { Loading, NotLoggedIn },
};
</script>

<template>
    <div v-if="!loading" class="flex justify-center items-center">
        ...
        <div v-if="fbData !== null" class="flex flex-col items-center mt-24">
            <p
                class="uppercase bg-blue-200 rounded-lg w-20 h-20 flex justify-center items-center font-bold text-2xl"
            >
            ...
            <div class="flex flex-col gap-2 my-4 text-lg">
                <p class="flex justify-between">
                    Email:
                    <span class="ml-1 hover:cursor-not-allowed">{{
                        fbData.email
                    }}</span>
                </p>
                <div class="flex justify-between">
                    First Name:<input
                        type="text"
                        v-model="fbData['first name']"
                        class="border px-2 ml-1"
                    />
                </div>
                <div class="flex justify-between">
                    Last Name:
            ...
            <button
                @click="handleSave"
                class="border py-3 px-7 rounded-full shadow font-semibold bg-sky-400 text-white mt-4"
            >
                Save Changes
            </button>
        </div>
...
export default {
    setup() {
        ...
        return { fbData, loading, handleSave, router };
    },
